...
public class DynamicComparator implements Comparator<Object>
{
    ...
    public int compare(final Object o1, final Object o2)
    {
        ...
        if (clazz1 == clazz2)
        {
            ...
            final Method m1 = comparableMethods.containsKey(clazz1) ? comparableMethods.get(clazz1) : getComparableMethod(clazz1);
            if (m1 != null)
            {
                try
                {
                    int result = ((Comparable) m1.invoke(o1, (Object[]) null)).compareTo(m1.invoke(o2, (Object[]) null));
                    if (reversedOrder)
                    {
                        result *= -1;
                    }
            ...
            else
            {
                ...
                else
                {
					 ...
					 * instances.
					 */
                    throw new IllegalArgumentException("Comparable property: " + propertyName + " of class: "
                            + String.valueOf(clazz1.getName()) + " does not exist and the class does not implement java.lang.Comparable");
                }
            }
        }
        else
        {
            ...
            final Method m1 = comparableMethods.containsKey(clazz1) ? comparableMethods.get(clazz1) : getComparableMethod(clazz1);
            final Method m2 = comparableMethods.containsKey(clazz2) ? comparableMethods.get(clazz2) : getComparableMethod(clazz2);

            if (m1 == null)
            {
                throw new IllegalArgumentException("Comparable property: " + propertyName + " of class: "
                        + String.valueOf(clazz1.getName()) + " does not exist!");
            }

            if (m2 == null)
            {
                throw new IllegalArgumentException("Comparable property: " + propertyName + " of class: "
                        + String.valueOf(clazz2.getName()) + " does not exist!");
            }

            if (m1.getReturnType() != m2.getReturnType())
            {
                throw new IllegalArgumentException( //
                        "Cannot compare property: " + String.valueOf(clazz1.getName()) + "#" //
                                + String.valueOf(m1.getName()) + ": " + String.valueOf(m1.getReturnType().getName()) //
                                + " with: " + String.valueOf(clazz2.getName()) + "#" //
                                + String.valueOf(m2.getName()) + ": " + String.valueOf(m2.getReturnType().getName()) //
                );
            }
            else
            {
                try
                {
                    int result = ((Comparable) m1.invoke(o1, (Object[]) null)).compareTo(m2.invoke(o2, (Object[]) null));
                    if (reversedOrder)
                    {
                        result *= -1;
                    }
